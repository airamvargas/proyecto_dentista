...
class Model_cie10 extends Model
{
    ...
    public function get_capitulo(){
        $query = "SELECT DISTINCT CAPITULO FROM hcv_cie10";
        $data = $this->db->query($query);
        return $data->getResult();
    }

    public function readEnfermedades($busqueda, $type){
    ...
    // Catalogo de procedimientos para nota de nutricion
	public function procedimientoNutricion(){
		return $this->asArray()
		->select('ID, NOMBRE, CAPITULO')
		->where('CAPITULO = ENFERMEDADES ENDOCRINAS, NUTRICIONALES Y METABOLICAS')
		->findAll();
	}

    // Catalogo de procedimientos para nota de nutricion
	public function procedimientoPsicologia(){
		return $this->asArray()
		->select('ID, NOMBRE, CAPITULO')
		->where('CAPITULO = TRASTORNOS MENTALES Y DEL COMPORTAMIENTO')
		->findAll();
	}
