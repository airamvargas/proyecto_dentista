<?php
    ...
    class Model_ventas extends Model
    {
        ...
        public function get_payments($id_sale){
            ...
            $builder->select('payments.id,payments.c_date,payments.amount,payments.status,cat_payments.name,cat_payments.description,
            file_patments.path');
        ...
        public function insert_dataventa($data){
            $db = \Config\Database::connect();
            $query = $this->db->table('data_venta');
            $query->insert($data);
           

        }

        public function hiden_cotization($data_cotization,$id){
                $db = \Config\Database::connect();
                 $builder = $db->table('cotization');
                 $builder->set($data_cotization);
                 $builder->where('id', $id);
                 $builder->update();
                 return $rc ="echo";
