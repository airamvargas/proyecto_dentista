...
$(document).on('click', '#procedimientos', function(){
    readProcedimientosPsico();
    $(".id_cita").val(id_cita);
});

$(document).on('submit', '#send_psicology', function () {
    $('#loader').toggle();
    const FORMDATA = new FormData($(this)[0]);
...
//Obtener datos de la nota medica
function readNota() {
    $('#loader').toggle();
    const URL = `${BASE_URL}${CONTROLADOR}/readNota`;
...
//Listado de procedimientos/intervenciones psicologicas
