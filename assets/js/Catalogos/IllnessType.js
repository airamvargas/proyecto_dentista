...
var dataTable = $('#tb_categoria').DataTable({
    ...
    ajax: {
        url:BASE_URL + '/Api/Catalogos/IllnessType/read',
        data: {},
        type: "post",
    },
    lengthMenu: [
...
$(document).on('click', '.up', function() {
    $('#loader').toggle();
    const url = `${BASE_URL}Api/Catalogos/IllnessType/readIllnessType`;
    ...
    $.ajax({
        ...
        success: function(success) {
            $('#nameIllnessType').val(success[0].name); 
            $('#description').val(success[0].description);
            $('#idIllnessType').val(success[0].id); 
