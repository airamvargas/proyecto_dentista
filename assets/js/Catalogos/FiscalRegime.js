...
var dataTable = $('#tb_categoria').DataTable({
    ...
    ajax: {
        url:BASE_URL + '/Api/Catalogos/FiscalRegime/read',
        data: {},
        type: "post",
    },
    lengthMenu: [
...
$(document).on('click', '.up', function() {
    $('#loader').toggle();
    const url = `${BASE_URL}Api/Catalogos/FiscalRegime/readFiscalRegime`;
    ...
    $.ajax({
        ...
        success: function(success) {
            $('#nameCode').val(success[0].code);
            $('#nameFiscalRegime').val(success[0].name); 
            $('#description').val(success[0].description);
            $('#idFiscalRegime').val(success[0].id); 
